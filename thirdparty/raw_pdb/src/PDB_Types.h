...
namespace PDB
{
	...
	struct PDB_NO_DISCARD SuperBlockV60
	{
		static const char MAGIC[43u];

		char fileMagic[43u];
		char padding;
		uint32_t blockSize;
		uint16_t freeBlockMapIndex;										// index of the free block map
		uint16_t blockCount;											// number of blocks in the file
		uint32_t directorySize;											// size of the stream directory in bytes
		uint32_t unknown;
		PDB_FLEXIBLE_ARRAY_MEMBER(uint16_t, directoryIndices);			// stream directory indices
	};
