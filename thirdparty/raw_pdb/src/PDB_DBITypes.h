...
#define PDB_V60_LENGTH_PREFIXED_STRING(name) \
union \
{ \
	struct { uint8_t length; PDB_FLEXIBLE_ARRAY_MEMBER(char, string); } vc60; \
	struct { PDB_FLEXIBLE_ARRAY_MEMBER(char, string); } vc70; \
} name
...
#pragma pack(push, 1)
					...
					struct
					{
						...
						uint32_t offset;
						uint16_t section;
						PDB_V60_LENGTH_PREFIXED_STRING(name);
					} S_PUB32;
					...
					struct
					{
						...
						PDB_V60_LENGTH_PREFIXED_STRING(version);
					} S_COMPILE3;
					...
					{
						uint8_t flags;
						PDB_V60_LENGTH_PREFIXED_STRING(strings);
					} S_ENVBLOCK;
