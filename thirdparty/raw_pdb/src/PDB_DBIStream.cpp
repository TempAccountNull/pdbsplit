...
PDB_NO_DISCARD PDB::ErrorCode PDB::HasValidDBIStream(const RawFile& file) PDB_NO_EXCEPT
{
	...
	else if (header.version != DBI::StreamHeader::Version::V70 && header.version != DBI::StreamHeader::Version::V60)
	{
...
PDB_NO_DISCARD PDB::ErrorCode PDB::DBIStream::HasValidImageSectionStream(const RawFile& /* file */) const PDB_NO_EXCEPT
{
	...
	uint16_t sectionHeaderStreamIndex = 0u;

	if (m_header.version <= DBI::StreamHeader::Version::V60)
	{
		const DBI::DebugHeaderV60& debugHeader = m_stream.ReadAtOffset<DBI::DebugHeaderV60>(debugHeaderOffset);
		sectionHeaderStreamIndex = debugHeader.sectionHeaderStreamIndex;
	}
	else
	{
	...
	if (sectionHeaderStreamIndex == DBI::DebugHeader::InvalidStreamIndex)
	{
		return ErrorCode::InvalidStreamIndex;
	}
...
PDB_NO_DISCARD PDB::ImageSectionStream PDB::DBIStream::CreateImageSectionStream(const RawFile& file) const PDB_NO_EXCEPT
{
	...
	return ImageSectionStream(file, sectionHeaderStreamIndex);
}
